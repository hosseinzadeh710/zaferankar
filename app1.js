...
function showCart() {
  ...
  if (regiserValues) {
    ...
    function seveInformtion() {
      ...
      if (ListElem.value && pardahktinput.value && ghabzinput.value && vazngolInputElem.value  && ghimatInputElem.value) {
...
function sabtkharidar() {
  ...
  var Registerr  = /^09((1[0-9]|9[013])|((3[0-1]|3[3-9])|0[1-5])|(2[0-2]))\d{7}$/g
...
function getUserlkharidar(AllDataUser){
  ...
  AllDataUser.map(function (user){
    ...
    Ulcontainer.insertAdjacentHTML ('afterbegin' , `<li class="nav-item">
    <button id="${user.id}" class="btnn m-auto mt-1 mb-1" type="button">${user.nameUser}</button>
</li>`)
   
   ListElem.insertAdjacentHTML ('afterbegin' , `<option id = "${user.id}" > ${user.nameUser} </option>`)
...
function createHeder() {
  ...
  newh6.id = 'h66'
  newh6.innerHTML = 'اطلاعات  <span style="color:#fa1212; font-size: large;">خریدار</span> رو وارد نماید!'
